...
for entry in result:
    ...
    c1.execute("INSERT INTO " + table +\
     " (USERID,LASTLATITUDE,LASTLONGITUDE,UPDATETIME,GEOFENCEID) VALUES (" \
        + str(entry[0]) + "," + str(entry[1]) + "," + str(entry[2]) + ",'" + \
        str(entry[3])[:-3] + "'," + str(entry[4]) +")")

# Need to commit the work before close the cursor.
c1.execute("COMMIT WORK")
conn.close()
