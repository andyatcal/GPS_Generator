...
class Simulator():
    ...
    def SimMultipleMoveRandomly(self, numWorker = 1):
        ...
        for i in range(0, 10): # Within 8 hrs
            for worker in self.workerLists:
                worker.velocity = np.array((random.uniform(-2.5, 2.5), random.uniform(-2.5, 2.5)))
                self.update(timeIncrement)
...
# Now let's write to the temp_server
# Server information
...
for entry in result:
    # print("INSERT INTO " + table + " (USERID,LASTLATITUDE,LASTLONGITUDE,UPDATETIME,GEOFENCEID) VALUES (" + str(entry[0]) + "," + str(entry[1]) + "," + str(entry[2]) + ",'" + str(entry[3])[:-3] + "'," + str(entry[4]) +")")
    c1.execute("INSERT INTO " + table + " (USERID,LASTLATITUDE,LASTLONGITUDE,UPDATETIME,GEOFENCEID) VALUES (" + str(entry[0]) + "," + str(entry[1]) + "," + str(entry[2]) + ",'" + str(entry[3])[:-3] + "'," + str(entry[4]) +")")

c1.execute('SELECT * FROM Temp_UserLocation')
print( c1.fetchall() )

# Delete the GPS generated by this program.
# c1.execute("DELETE FROM Temp_UserLocation WHERE USERID = 5")
conn.close()
